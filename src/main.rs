...
#[cfg(feature = "sshauth")]
use auth_ssh::maybe_create_ssh_authenticator;
#[derive(Debug)]
struct AppError {
    status: StatusCode,
    message: String,
...
/// Server-side nonce store for the redirect-based auth flow.
///
/// The server generates a nonce and redirects the client to a URL that
/// includes it. The client signs the full URL (including the nonce) and
/// reconnects. The server verifies the nonce was generated by itself,
/// preventing cross-server replay attacks.
struct ServerNonceStore {
    pending: HashMap<String, std::time::Instant>,
    max_age: std::time::Duration,
}

impl ServerNonceStore {
    fn new() -> Self {
        Self {
            pending: HashMap::new(),
    ...
    fn generate(&mut self) -> String {
        ...
        self.pending
            .insert(nonce.clone(), std::time::Instant::now());
        nonce
    }

    fn verify_and_consume(&mut self, nonce: &str) -> bool {
        self.prune();
    ...
    fn prune(&mut self) {
        ...
        self.pending
            .retain(|_, created| now.duration_since(*created) < self.max_age);
    }
}
...
async fn auth_middleware(
    ...
    let auth_header = match request.headers().get("authorization") {
        ...
        None => {
            // No auth header → generate nonce and redirect so the client
            // can sign the URL that includes the server-generated nonce.
            let nonce = state.nonces.lock().unwrap().generate();
            let path_and_query = request
                .uri()
                .path_and_query()
                .map_or(request.uri().path().to_string(), |pq| pq.to_string());
            let redirect_url = if path_and_query.contains('?') {
                format!("{path_and_query}&nonce={nonce}")
            } else {
                format!("{path_and_query}?nonce={nonce}")
            };
            return (
    ...
    // Extract and verify the server-generated nonce from the query string
    let nonce = request
        .uri()
        .query()
        .and_then(|q| q.split('&').find_map(|pair| pair.strip_prefix("nonce=")))
        .map(String::from);

    let Some(nonce) = nonce else {
        return (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({"error": "missing nonce query parameter"})),
        )
            .into_response();
    };

    if !state.nonces.lock().unwrap().verify_and_consume(&nonce) {
        return (
            StatusCode::UNAUTHORIZED,
            axum::Json(json!({"error": "invalid or expired nonce"})),
        )
            .into_response();
    }
...
struct AppState {
    ...
    nonces: Arc<std::sync::Mutex<ServerNonceStore>>,
}
...
fn create_router(
    ...
    let shared_state = AppState {
        ...
        nonces: Arc::new(std::sync::Mutex::new(ServerNonceStore::new())),
    };
